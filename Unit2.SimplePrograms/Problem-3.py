...
epsilion = 0.01
monthly_payment = (high + low) / 12


while abs(updated_balance) >= epsilion:
    updated_balance = balance
    total_paid = 0
    for i in range(12):
        unpaid_balance = updated_balance - monthly_payment
        updated_balance = unpaid_balance + unpaid_balance * monthly_interest
    if updated_balance > epsilion:
        low = monthly_payment
    elif updated_balance < epsilion:
        high = monthly_payment
    else:
        break
    monthly_payment = (high + low) / 2

print('Lowest Payment: {:.2f}'.format(monthly_payment))